...
def test_mod_with_file_types():
    lc = list(Repository('test-repos/git-4/', only_modifications_with_file_types=['.java']).traverse_commits())
...
def test_only_in_main_branch():
    lc = list(Repository('test-repos/git-5/').traverse_commits())
...
def test_multiple_filters():
    lc = list(Repository('test-repos/git-5/', only_no_merge=True).traverse_commits())
...
def test_no_filters():
    lc = list(Repository('test-repos/git-4/').traverse_commits())
...
def test_only_in_branch():
    lc = list(Repository('test-repos/git-5/', only_in_branch='branch2').traverse_commits())
    assert len(lc) == 6
...
def test_only_in_branches():
    ...
    assert len(list(Repository('test-repos/test8/').traverse_commits())) == 3
    # only analyze b2
    assert len(list(Repository('test-repos/test8/', only_in_branch='b2').traverse_commits())) == 4
    # only analyze b1
    assert len(list(Repository('test-repos/test8/', only_in_branch='b1').traverse_commits())) == 5


def test_only_in_branch_not_exist():
    with pytest.raises(Exception):
        list(Repository('test-repos/git-5/', only_in_branch='branch7').traverse_commits())


def test_only_authors():
    lc = list(Repository('test-repos/git-10/', only_authors=["Maurício Aniche"]).traverse_commits())
    assert len(lc) == 4

    lc = list(Repository('test-repos/git-10/', only_authors=["ishepard"]).traverse_commits())
    assert len(lc) == 1
...
def test_only_commits():
    ...
    lc = list(Repository('test-repos/git-10/',
    ...
    lc = list(Repository('test-repos/git-10/', only_commits=["fake hash"]).traverse_commits())
    assert len(lc) == 0

    only_commits = len(list(Repository('test-repos/git-10/',
                                       only_commits=["4e669cb4f69245dc669e116517d80d038d8e0434",
                                                           "29e929fbc5dc6a2e9c620069b24e2a143af4285f",
                                                           "8986af2a679759e5a15794f6d56e6d46c3f302f1",
                                                           "8169f76a3d7add54b4fc7bca7160d1f1eede6eda",
    ...
    total_commits = len(list(Repository('test-repos/git-10/').traverse_commits()))
