...
from pydriller import Repository, GitGP
...
@pytest.fixture
def repo(path):
    return list(Repository(path_to_repo=path).traverse_commits())
...
@pytest.mark.skipif(sys.version_info < (3, 8) and sys.platform == "win32", reason="requires Python3.8 on windows")
