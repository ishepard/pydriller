...
def test_only_no_merge():
    lc = list(Repository('test-repos/branches_merged',
...
def test_only_in_branch():
    lc = list(Repository('test-repos/branches_not_merged',
...
def test_only_in_branches():
    ...
    assert len(list(Repository('test-repos/branches_not_merged')
                    .traverse_commits())) == 3
    # only analyze b2
    assert len(list(Repository('test-repos/branches_not_merged',
                               only_in_branch='b2')
...
def test_only_in_branch_not_exist():
    with pytest.raises(Exception):
        list(Repository('test-repos/branches_not_merged', only_in_branch='branch2')
             .traverse_commits())


def test_only_authors():
    lc = list(Repository('test-repos/multiple_authors',
...
def test_only_commits():
    ...
    lc = list(Repository('test-repos/complex_repo',
                         only_commits=["953737b199de233896f00b4d87a0bc2794317253", "ffccf1e7497eb8136fd66ed5e42bef29677c4b71"]).traverse_commits())
    assert len(lc) == 2
    ...
    total_commits = len(list(Repository('test-repos/complex_repo').traverse_commits()))
...
def test_single_commit_head():
    ...
    lc_head = list(Repository('test-repos/complex_repo', single="HEAD").traverse_commits())
...
def test_only_releases_wo_releases():
    lc = list(Repository('test-repos/complex_repo', only_releases=True).traverse_commits())

    assert len(lc) == 0


def test_date_order():
    date_order = list(Repository('test-repos/order', order='date-order').traverse_commits())
    author_date_order = list(Repository('test-repos/order', order='author-date-order').traverse_commits())
...
def test_topo_order():
    topo_order = list(Repository('test-repos/order', order='topo-order').traverse_commits())
...
def test_should_visit_descendent_order():
    lc = list(Repository('test-repos/small_repo',
...
def test_include_refs():
    commits_no_refs = list(Repository('test-repos/branches_not_merged/',
    ...
    commits_with_refs = list(Repository('test-repos/branches_not_merged/',
...
def test_include_remotes():
    ...
    commits_before_fetch = [commit.hash for commit in Repository('test-repos/pydriller/').traverse_commits()]
    ...
    repo.git.fetch('--all')
    commits_after_fetch = [commit.hash for commit in Repository('test-repos/pydriller/').traverse_commits()]
    ...
    commits_with_remotes = [commit.hash for commit in Repository('test-repos/pydriller/', include_remotes=True).traverse_commits()]
