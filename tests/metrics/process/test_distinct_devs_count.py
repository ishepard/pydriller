...
def test_distinct_devs_from_hash():
    count = ProcessMetrics().distinct_dev_count(path_to_repo='test-repos/git-1', filepath='unexisting.java', commit_hash='ffccf1e7497eb8136fd66ed5e42bef29677c4b71')
    assert count == 0

    count = ProcessMetrics().distinct_dev_count(path_to_repo='test-repos/git-1', filepath='Matricula.java', commit_hash='ffccf1e7497eb8136fd66ed5e42bef29677c4b71')
    assert count == 1
...
def test_distinct_devs_til_creation_file():
    ...
    count = ProcessMetrics().distinct_dev_count(path_to_repo='test-repos/git-1', filepath='Secao.java', commit_hash='f0dd1308bd904a9b108a6a40865166ee962af3d4')
    assert count == 1

def test_distinct_devs_from_latest_remote():
    count = ProcessMetrics().distinct_dev_count(path_to_repo='https://github.com/ishepard/pydriller', filepath='domain/developer.py', commit_hash='fdf671856b260aca058e6595a96a7a0fba05454b')
    assert count == 1
